...
export async function SiteFooter({ footer, headerCtas }: Props) {
  ...
  const accessLinks = listVisibleWebsiteQuickLinks(ctas, 'header');
        ...
        <div className="footer-contact">
          ...
          {accessLinks.length ? (
            <div className="footer-cta-row" aria-label="App and portal links">
              {accessLinks.map(({ key, link }) => (
                <QuickLinkAnchor key={key} link={link} className={`footer-cta footer-cta-${key}`}>
                  <QuickLinkIcon linkKey={key} className="footer-cta-icon" />
