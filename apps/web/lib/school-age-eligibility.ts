/**
 * Keep in sync with apps/api/src/modules/school-admissions/school-age-eligibility.ts
 * Calendar dates only (UTC midnight). Do not treat DOB as a local timestamp.
 */
...
export function eligibleDobRangeUtc(
  ...
  const maxDob = new Date(
    Date.UTC(census.getUTCFullYear() - minAgeYears, census.getUTCMonth(), census.getUTCDate()),
  );
...
export function schoolAgeIneligibleMessage(
  ...
  const { minDob, maxDob } = eligibleDobRangeUtc(census, minAgeYears, maxAgeYearsExclusive);
...
export function evaluateSchoolAgeEligibility(
  ...
  const maxYearsExclusive = maxAgeYearsExclusive ?? TPS_KG_2027_MAX_AGE_YEARS_EXCLUSIVE;
  ...
  const { minDob, maxDob } = eligibleDobRangeUtc(census, minYears, maxYearsExclusive);
  const age = ageAsOf(dob, census);
  const inRange = compareUtcDateOnly(dob, minDob) >= 0 && compareUtcDateOnly(dob, maxDob) <= 0;
  ...
  if (!inRange) {
    return {
      ...
      minDob: minIso,
      maxDob: maxIso,
      message: schoolAgeIneligibleMessage(censusDate, minYears, maxYearsExclusive),
    };
  }
  return {
    age,
