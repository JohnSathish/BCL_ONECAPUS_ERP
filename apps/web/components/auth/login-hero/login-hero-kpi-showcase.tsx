...
function KpiValue({
  ...
  if (isTextOnly) {
    return (
      <p className={`text-sm font-semibold ${onLight ? 'text-[#0b1f3a]' : 'text-white'}`}>
        {metric.label}
      </p>
    );
  }
  ...
  return (
    <>
      <p
        className={`login-hero-kpi-value text-xl font-bold tabular-nums tracking-tight sm:text-2xl ${
          onLight ? '' : 'text-white'
        }`}
      >
        {metric.prefix}
      ...
      <p
        className={`login-hero-kpi-label mt-0.5 text-[10px] font-medium uppercase tracking-wide ${
          onLight ? '' : 'text-white/75'
        }`}
      >
        {metric.label}
      </p>
...
export function LoginHeroKpiShowcase() {
        ...
        <ul className="login-hero-kpi-grid grid grid-cols-2 gap-2">
          {KPI_METRICS.filter((m) => !m.textOnly)
            ...
            .map((m) => (
              <li
                key={m.label}
                className="login-hero-kpi-cell login-hero-kpi-panel rounded-lg px-3 py-2.5"
              >
                <KpiValue metric={m} animate={false} onLight />
              </li>
            ))}
        </ul>
      </div>
  ...
  return (
    <div ref={ref} className="login-hero-section w-full">
      <motion.div
        className="login-hero-kpi-panel rounded-xl px-4 py-3"
      ...
      >
        ...
        <AnimatePresence mode="wait">
          ...
          >
            <KpiValue metric={metric} animate={inView} onLight />
