...
export function requiredMajorPaperCount(categoryCounts: Record<string, number>): number {
  return categoryCounts.MAJOR ?? 0;
}

function compareNullableIndex(a: number | null | undefined, b: number | null | undefined) {
...
export function dedupeMajorPaperRowsByCourse<T extends MajorPaperRow>(rows: T[]): T[] {
...
export function resolveMajorPaperForSlot<T extends MajorPaperRow>(
  ...
  const exact = sorted.find(
    (row) => row.majorPaperIndex === paperIndex && !usedCourseIds.has(row.courseId),
  );
  if (exact) return exact;

  for (const row of sorted) {
    if (usedCourseIds.has(row.courseId)) continue;
    if (row.majorPaperIndex != null && row.majorPaperIndex !== paperIndex) continue;
    return row;
  }
