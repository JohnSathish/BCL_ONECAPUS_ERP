/** Mirrors server defaults in PasswordPolicyService (tenant min length may be up to 12). */
...
export function evaluatePasswordPolicy(
  ...
  const checks: PasswordPolicyCheck[] = [
    {
      id: 'length',
      label: `At least ${rules.minLength} characters (max ${rules.maxLength})`,
...
export function passwordPolicySummary(
  ...
  return `Use ${rules.minLength}–${rules.maxLength} characters with uppercase, lowercase, a number, and a special character. Do not reuse your last ${rules.historyCount} passwords.`;
}
