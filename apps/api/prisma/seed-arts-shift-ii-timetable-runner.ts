/**
 * Populate DBC Arts Shift II timetable (Sem 3 & 5, Jun–Jul 2026).
 * Run: npx ts-node --transpile-only prisma/seed-arts-shift-ii-timetable-runner.ts
 */
...
async function main() {
  ...
  const result = await seedArtsShiftIiTimetable({
    prisma,
    tenantId: tenant.id,
