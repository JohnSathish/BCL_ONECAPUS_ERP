/**
 * Map the office Day Shift B.Sc Semester 3 register onto the official
 * Sem 3 admission import template.
 *
 *   npx ts-node --transpile-only scripts/map-sem3-day-bsc-import.ts
 */
...
const PAPER_ALIASES: Record<string, string> = {
  ...
  'critical reading': 'Critical Reading (Science)',
  'critical reading science': 'Critical Reading (Science)',
...
function pickSourceSheet(book: ExcelJS.Workbook): ExcelJS.Worksheet {
  const named =
    book.getWorksheet('III SEM B.SC 2025 (2)') ??
    book.worksheets.find((sheet) => /b\.?sc/i.test(sheet.name));
...
function uniqueEmail(seed: string, used: Set<string>): string {
  const base = emailBase(seed) || 'sem3bsc';
...
async function main() {
  ...
  for (let rowNumber = 3; rowNumber <= sourceSheet.rowCount; rowNumber += 1) {
    ...
    if (email.includes('@')) {
      ...
      email = uniqueEmail(roll || `sem3bsc${rowNumber}`, usedEmails);
      flags.push({
        severity: 'warn',
    ...
    let abc = digitsOnly(srcAny(row, headers, ['ABC NO', 'ABC ID', 'ABC_ID']));
  ...
  students.addRow(
    studentHeaders.map((header) => {
      ...
      if (header === 'AEC Paper') {
        return 'Day B.Sc has several AEC options — filled from office Excel';
      }
      return (
        SEM3_ADMISSION_TEMPLATE_HELPERS[
  ...
  notes.addRow(['Day Shift B.Sc Semester 3 — mapped ERP import']);
  notes.addRow([]);
  ...
  notes.addRow(['AEC', 'Auto-assigned / filled Critical Reading (Science)']);
  notes.addRow([
    'Major in office Excel',
    'ZOOLOGY / MATHS / BOTANY / CHEMISTRY / PHYSICS mapped to Zoology, Mathematics, Botany, Chemistry, Physics.',
  ]);
  ...
  const lines = [
    'Day Shift B.Sc Semester 3 — mapping report',
    '='.repeat(72),
    `Source: ${SOURCE}`,
    `Output: ${OUTPUT}`,
