/**
 * Map the office Day Shift B.Com Semester 5 register onto the official
 * Sem 5 admission import template.
 *
 *   npx ts-node --transpile-only scripts/map-sem5-day-bcom-import.ts
 */
...
function headerCols(
  ...
  const collapsed = collapseSpaces(name);
  if (headers.has(collapsed)) return headers.get(collapsed);
  const want = normalizeLabel(name);
  for (const [key, cols] of headers) {
    if (normalizeLabel(key) === want) return cols;
  }
  return undefined;
}
...
function srcAt(
  ...
  const cols = headerCols(headers, name);
...
function uniqueEmail(seed: string, used: Set<string>): string {
  const base = emailBase(seed) || 'sem5bcom';
...
function mapCommerceDepartment(raw: string): string | undefined {
  const key = normalizeLabel(raw);
  if (!key) return undefined;
  if (COMMERCE_PAPER_TITLES.includes(key)) return 'Commerce';
  return canonicalDbcMajorName(raw);
}

function mapShift(
  raw: string,
  options: { silentBlank?: boolean } = {},
): {
  code: string;
  include: boolean;
  note?: string;
  ...
  if (!key) {
    return {
      code: 'DAY',
      include: true,
      note: options.silentBlank
        ? undefined
        : 'Shift blank — defaulted to DAY (this is the Day workbook)',
    };
  }
  if (key.includes('day') && !key.includes('ms ds')) {
    return { code: 'DAY', include: true };
  }
  if (key === 'ms ds' || key.includes('ms-ds') || key === 'msds') {
    return {
      code: 'DAY',
      include: true,
      note: 'Office listed MS-DS — imported as DAY per this Day workbook',
    };
  }
  if (key.includes('morning')) {
    return {
      code: 'DAY',
      include: true,
      note: 'Office listed Morning Shift — imported as DAY (not in the Morning Sem 5 file)',
    };
  }
  if (key.includes('evening') || key.includes('shift ii')) {
    return {
      code: 'DAY',
      include: true,
      note: 'Office listed Evening Shift; Arts Shift II is inactive in ERP so imported as DAY',
    };
  }
  return {
    code: 'DAY',
    include: true,
    note: `Unknown shift "${raw}" defaulted to DAY`,
  };
}
...
async function main() {
  ...
  for (let rowNumber = 3; rowNumber <= sourceSheet.rowCount; rowNumber += 1) {
    ...
    const roll = collapseSpaces(
      srcAny(row, headers, ['ROLL NO', 'ROLL NO.', 'Roll No.', 'Roll No']),
    ).toUpperCase();
    const majorRaw = srcAny(row, headers, ['MAJOR', 'Major Subject']);
    const minorRaw = srcAny(row, headers, ['MINOR', 'Minor Subject']);
    const major = mapCommerceDepartment(majorRaw) ?? titleCaseWords(majorRaw);
    const minor =
      mapCommerceDepartment(minorRaw) ??
      canonicalDbcMajorName(minorRaw) ??
      titleCaseWords(minorRaw);
    const programme = major === 'Commerce' ? 'BCOM' : '';
    if (!programme) {
      flags.push({
        severity: 'block',
        reason: `Unknown major "${majorRaw}" — B.Com Sem 5 expects Commerce`,
      });
    }
    if (!minor) {
    ...
    const hasShiftColumn = Boolean(headerCols(headers, 'Shift'));
    const shiftInfo = mapShift(srcAt(row, headers, 'Shift'), {
      silentBlank: !hasShiftColumn,
    });
    ...
    if (email.includes('@')) {
      ...
      email = uniqueEmail(roll || `sem5bcom${rowNumber}`, usedEmails);
      flags.push({
        severity: 'warn',
    ...
    let abc = digitsOnly(
      srcAny(row, headers, ['ABC ID', 'ABC_ID', 'ABC ID NO', 'ABC NO', 'ABC']),
    );
  ...
  const eveningAsDayCount = importRows.filter((e) =>
    e.flags.some((f) => f.reason.includes('Evening Shift')),
  ).length;
  const morningAsDayCount = importRows.filter((e) =>
    e.flags.some((f) => f.reason.includes('Morning Shift')),
  ).length;
  const msDsAsDayCount = importRows.filter((e) =>
    e.flags.some((f) => f.reason.includes('MS-DS')),
  ).length;
  const blankAsDayCount = importRows.filter((e) =>
  ...
  notes.addRow(['Day Shift B.Com Semester 5 — mapped ERP import']);
  notes.addRow([]);
  ...
  notes.addRow(['Evening students mapped as Day', eveningAsDayCount]);
  notes.addRow(['Morning students mapped as Day', morningAsDayCount]);
  notes.addRow(['MS-DS students mapped as Day', msDsAsDayCount]);
  notes.addRow(['Blank-shift students mapped as Day', blankAsDayCount]);
  ...
  notes.addRow([
    'Internship',
    'Filled from major as COM-303 — Internship (no internship column in office Excel)',
  ]);
  notes.addRow([
    'Major / Minor',
    'Office Major Subject "Human Resource Management" is a Commerce paper → Major Department Commerce. Office MINOR "ECONOMICS" kept as Economics (official Commerce pair).',
  ]);
  ...
  const lines = [
    'Day Shift B.Com Semester 5 — mapping report',
    '='.repeat(72),
    `Source: ${SOURCE}`,
    `Output: ${OUTPUT}`,
    ...
    `  Day: ${importRows.filter((e) => e.values.Shift === 'DAY').length}`,
    `  Morning listed in Excel, mapped as Day: ${morningAsDayCount}`,
    `  Evening listed in Excel, mapped as Day: ${eveningAsDayCount}`,
    `  MS-DS listed in Excel, mapped as Day: ${msDsAsDayCount}`,
    `  Blank shift, mapped as Day: ${blankAsDayCount}`,
