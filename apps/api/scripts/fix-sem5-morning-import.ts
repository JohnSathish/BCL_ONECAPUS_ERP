...
const SOURCE = path.join(
  ...
  'Morning Shift',
  '5th Semester',
  '5th Sem Morning Shift Final Import02.xlsx',
);
...
async function loadSem5MinorByMajor(prisma: PrismaClient) {
  ...
  try {
    ...
    const map = await sem5Curriculum.buildTenantMinorByMajor(
      tenant.id,
      morningShift?.id,
    );
    return map;
...
async function main() {
  ...
  for (let rowNumber = 3; rowNumber <= sheet.rowCount; rowNumber += 1) {
    ...
    if (!registration) {
      registration = `TMP-S5-${String(rowNumber).padStart(4, '0')}`;
      notes.push(
    ...
    if (!email || !email.includes('@')) {
      const seed = toEmailBase(pickFirst(roll, registration, `s5${rowNumber}`));
      let candidate = `${seed || `s5${rowNumber}`}@student.donboscocollege.ac.in`;
      let index = 1;
      while (db.dbEmails.has(candidate) || fileEmails.has(candidate)) {
        index += 1;
        candidate = `${seed || `s5${rowNumber}`}${index}@student.donboscocollege.ac.in`;
      }
      email = candidate;
      generatedEmail += 1;
      ...
      let candidate = `${prefix || `s5${rowNumber}`}@student.donboscocollege.ac.in`;
      let index = 1;
      while (db.dbEmails.has(candidate) || fileEmails.has(candidate)) {
        index += 1;
        candidate = `${prefix || `s5${rowNumber}`}${index}@student.donboscocollege.ac.in`;
      }
      notes.push(
  ...
  const report = [
    'Sem 5 Morning Shift Import Fix Report',
    '='.repeat(60),
    `Source: ${SOURCE}`,
    `Output: ${OUTPUT}`,
    '',
    ...
    `Department code cleared (used major mapping only): ${departmentCleared}`,
