/**
 * Map the office Day Shift B.A. Semester 5 register onto the official
 * Sem 5 admission import template.
 *
 *   npx ts-node --transpile-only scripts/map-sem5-day-ba-import.ts
 */
...
function uniqueEmail(seed: string, used: Set<string>): string {
  const base = emailBase(seed) || 'sem5day';
...
function mapShift(raw: string): {
  ...
  if (!key) {
    return {
      code: 'DAY',
      include: true,
      note: 'Shift blank — defaulted to DAY (this is the Day workbook)',
    };
  }
...
async function main() {
  ...
  for (let rowNumber = 3; rowNumber <= sourceSheet.rowCount; rowNumber += 1) {
    ...
    if (email.includes('@')) {
      ...
      email = uniqueEmail(roll || `sem5day${rowNumber}`, usedEmails);
      flags.push({
        severity: 'warn',
  ...
  notes.addRow(['Day Shift B.A. Semester 5 — mapped ERP import']);
  notes.addRow([]);
  ...
  const lines = [
    'Day Shift B.A. Semester 5 — mapping report',
    '='.repeat(72),
    `Source: ${SOURCE}`,
    `Output: ${OUTPUT}`,
