/**
 * Fix B.Com Sem 5 bulk import Excel and optionally validate against ERP.
 *
 *   npx ts-node --transpile-only scripts/fix-sem5-bcom-import.ts
 *   npx ts-node --transpile-only scripts/fix-sem5-bcom-import.ts --validate
 */
...
const SOURCE = path.join(
  ...
  'Import Live 1-3-5',
  'V SEM',
  'B.Com 5 Semester students Bulk Import.xlsx',
);
const OUTPUT = SOURCE.replace('.xlsx', ' - READY TO IMPORT.xlsx');
const REPORT = SOURCE.replace('.xlsx', ' - FIX REPORT.txt');
...
async function loadSem5MinorByMajor(prisma: PrismaClient) {
  ...
  try {
    ...
    return sem5Curriculum.buildTenantMinorByMajor(tenant.id, dayShift?.id);
  } finally {
    await app.close();
  }
}
...
function isTemplateSampleRow(input: {
  ...
  const programme = upper(input.programme);
  if (programme && programme !== 'BCOM') return true;
  if (input.reg.toUpperCase() === 'REG2026001') return true;
  if (input.email.toLowerCase() === 'student@example.edu') return true;
  if (!input.roll && !programme) return true;
  return false;
}
...
async function main() {
  ...
  if (sem5MajorCol && sem5InternshipCol) {
  ...
  for (let rowNumber = 3; rowNumber <= sheet.rowCount; rowNumber += 1) {
    ...
    const admissionRaw = cellText(
      row.getCell(headers.get('Admission Number')!).value,
    );
    const applicationRaw = cellText(
      row.getCell(headers.get('Application Number')!).value,
    );
    ...
    if (upper(programmeRaw) !== 'BCOM') {
      row.getCell(programmeCol).value = 'BCOM';
      programmeFixed += 1;
    }
    ...
    if (!registration) {
      registration = `TMP-BCOM5-${String(rowNumber).padStart(4, '0')}`;
      notes.push(
    ...
    if (!email || !email.includes('@')) {
      const seed = toEmailBase(
        pickFirst(roll, registration, `bcom5${rowNumber}`),
      );
      let candidate = `${seed || `bcom5${rowNumber}`}@student.donboscocollege.ac.in`;
      let index = 1;
      while (db.dbEmails.has(candidate) || fileEmails.has(candidate)) {
        index += 1;
        candidate = `${seed || `bcom5${rowNumber}`}${index}@student.donboscocollege.ac.in`;
      }
      email = candidate;
      generatedEmail += 1;
      ...
      let candidate = `${prefix || `bcom5${rowNumber}`}@student.donboscocollege.ac.in`;
      let index = 1;
      while (db.dbEmails.has(candidate) || fileEmails.has(candidate)) {
        index += 1;
        candidate = `${prefix || `bcom5${rowNumber}`}${index}@student.donboscocollege.ac.in`;
      }
      notes.push(
    ...
    if (sem5MajorCol && sem5MinorCol) {
      const major = cellText(row.getCell(sem5MajorCol).value);
      const minor = cellText(row.getCell(sem5MinorCol).value);
      if (major && !minor) {
  ...
  const report = [
    'B.Com Sem 5 Day Shift Import Fix Report',
    '='.repeat(60),
    `Source: ${SOURCE}`,
    `Output: ${OUTPUT}`,
    '',
    ...
    `Programme set to BCOM: ${programmeFixed}`,
    ...
    `Sem 5 minor filled from curriculum: ${sem5MinorFilled}`,
  ...
  if (validate) {
    ...
    try {
      ...
      if (invalid.length > 20) {
        console.log(`... and ${invalid.length - 20} more invalid rows`);
      }
    } finally {
      await app.close();
    }
