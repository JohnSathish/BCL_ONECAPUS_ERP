/**
 * Map the office Day Shift B.Sc Semester 5 register onto the official
 * Sem 5 admission import template.
 *
 *   npx ts-node --transpile-only scripts/map-sem5-day-bsc-import.ts
 */
...
function uniqueEmail(seed: string, used: Set<string>): string {
  const base = emailBase(seed) || 'sem5bsc';
...
function mapScienceDepartment(raw: string): string | undefined {
...
async function main() {
  ...
  for (let rowNumber = 3; rowNumber <= sourceSheet.rowCount; rowNumber += 1) {
    ...
    const majorRaw = srcAny(row, headers, [
      'MAJOR',
      'Major Subject',
      'Major Subj',
    ]);
    const minorRaw = srcAny(row, headers, ['MINOR', 'Minor Subject']);
    const major = mapScienceDepartment(majorRaw) ?? titleCaseWords(majorRaw);
    const minor = mapScienceDepartment(minorRaw) ?? titleCaseWords(minorRaw);
    const programme = PROGRAMME_BY_MAJOR[major] ?? '';
    if (!programme) {
      flags.push({
        severity: 'block',
        reason: `Unknown major "${majorRaw}" — cannot assign B.Sc programme`,
      });
    }
    if (!minor) {
    ...
    if (email.includes('@')) {
      ...
      email = uniqueEmail(roll || `sem5bsc${rowNumber}`, usedEmails);
      flags.push({
        severity: 'warn',
  ...
  notes.addRow(['Day Shift B.Sc Semester 5 — mapped ERP import']);
  notes.addRow([]);
  ...
  notes.addRow([
    'Major / Minor',
    'Office department names mapped to Botany / Chemistry / Mathematics / Physics / Zoology. Chemistry + Botany is unofficial and kept under a named Principal exception.',
  ]);
  ...
  const lines = [
    'Day Shift B.Sc Semester 5 — mapping report',
    '='.repeat(72),
    `Source: ${SOURCE}`,
    `Output: ${OUTPUT}`,
