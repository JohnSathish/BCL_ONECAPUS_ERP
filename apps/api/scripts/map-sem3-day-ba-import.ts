/**
 * Map the office Day Shift B.A. Semester 3 register onto the official
 * Sem 3 admission import template.
 *
 *   npx ts-node --transpile-only scripts/map-sem3-day-ba-import.ts
 */
...
const SOURCE =
  ...
  path.join(
    ...
    'Import Live 1-3-5',
    'III Sem BA',
    'III SEM ARTS-DAY SHIFT 2025 NEW.xlsx',
  );
...
const PAPER_ALIASES: Record<string, string> = {
  ...
  'web designing': 'Web Designing – I',
  'physics around us': 'Physics Around Us',
  'bee keeping': 'Bee Keeping – I',
  'introduction to academic writing': 'Introduction to Academic Writing (Arts)',
  'introduction to academic writing arts':
    'Introduction to Academic Writing (Arts)',
};
...
function pickSourceSheet(book: ExcelJS.Workbook): ExcelJS.Worksheet {
  const named =
    book.getWorksheet('III SEM B.A DS -2025-NEW') ??
    book.worksheets.find((sheet) => /b\.?a|arts/i.test(sheet.name));
...
function uniqueEmail(seed: string, used: Set<string>): string {
  const base = emailBase(seed) || 'sem3ba';
...
async function main() {
  ...
  for (let rowNumber = 3; rowNumber <= sourceSheet.rowCount; rowNumber += 1) {
    ...
    if (!programme) {
      flags.push({
        severity: 'block',
        reason: `Unknown major "${majorRaw}" — cannot assign B.A. programme`,
      });
    }
    ...
    if (
      ...
      flags.push({
        severity: 'warn',
        reason:
          'English major + English Proficiency MDC is excluded in the Day Sem 3 pool rules',
      });
    }
    if (
      major === 'Education' &&
      ...
      flags.push({
        severity: 'warn',
        reason:
          'Education major + Development of Education in North-East India MDC is excluded in the Day Sem 3 pool rules',
      });
    }
    ...
    if (email.includes('@')) {
      ...
      email = uniqueEmail(roll || `sem3ba${rowNumber}`, usedEmails);
      flags.push({
        severity: 'warn',
    ...
    if (!mobile) mobile = digitsOnly(srcAny(row, headers, ['Contact No']));
    ...
    let abc = digitsOnly(
      srcAny(row, headers, ['ABC', 'ABC ID', 'ABC NO', 'ABC_ID']),
    );
    ...
    const officeGrp = collapseSpaces(
      srcAny(row, headers, ['Grp', 'GRP', 'Section Code', 'Section']),
    ).toUpperCase();
    if (officeGrp && officeGrp !== 'A') {
      flags.push({
        severity: 'warn',
        reason: `Office Grp ${officeGrp} not created in ERP Sem 3 sections (only A exists) — left Section Code blank`,
      });
    }
  ...
  students.addRow(
    studentHeaders.map((header) => {
      ...
      if (header === 'AEC Paper') {
        return 'Day B.A. has several AEC options — filled from office Excel';
      }
      return (
        SEM3_ADMISSION_TEMPLATE_HELPERS[
  ...
  notes.addRow(['Day Shift B.A. Semester 3 — mapped ERP import']);
  notes.addRow([]);
  ...
  notes.addRow(['AEC', 'Filled Introduction to Academic Writing (Arts)']);
  notes.addRow([
    'Section Code',
    'Office Grp A kept as Section A. Grp B left blank because ERP Sem 3 only has section A.',
  ]);
  ...
  const lines = [
    'Day Shift B.A. Semester 3 — mapping report',
    '='.repeat(72),
    `Source: ${SOURCE}`,
    `Output: ${OUTPUT}`,
