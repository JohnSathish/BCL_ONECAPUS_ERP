...
export type Sem2ImportCurriculumCatalog = {
  ...
  semesterSequence: 2;
  shiftId: string | null;
  majorDepartments: Sem2MajorDepartmentOption[];
  mdcPapers: Sem2PaperOption[];
  aecPapers: Sem2PaperOption[];
  secPapers: Sem2PaperOption[];
  vacPapers: Sem2PaperOption[];
...
@Injectable()
export class Sem2ImportCurriculumService {
  constructor(
  ...
  async resolveProgramVersion(
    ...
    if (!programme) {
      throw new BadRequestException(
        'Programme is required to generate the Semester 2 import template',
      );
    }
  ...
  async buildCatalog(
  ...
  ): Promise<Sem2ImportCurriculumCatalog> {
    const semesterSequence = 2;
    ...
    const vacPapers = this.buildCategoryPaperOptions(byCategory('VAC'));
    if (!vacPapers.length) {
      throw new BadRequestException(
        'Semester 2 VAC offerings are not configured for this programme and shift.',
      );
    }
  ...
  async listEligibleMinorsForMajor(
    ...
    return {
      ...
      vacPapers: catalog.vacPapers,
      eligibleMinors: minors,
    };
  }

  resolveMajorDepartment(
    catalog: Sem2ImportCurriculumCatalog,
    input: string,
  ): Sem2MajorDepartmentOption | undefined {
  ...
  resolveCategoryPaper(
    options: Sem2PaperOption[],
    input: string,
    categoryLabel: string,
  ): Sem2PaperOption | undefined {
    const normalized = this.normalizeLabel(input);
  ...
  private toPaperOption(offering: CurriculumOffering): Sem2PaperOption {
    return {
