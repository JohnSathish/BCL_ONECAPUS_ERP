export const SCHOOL_CASTE_CATEGORY_CODES = [
  'GENERAL_UR',
  'SC',
  'ST',
  'OBC',
...
export type SchoolCasteCategoryCode =
  (typeof SCHOOL_CASTE_CATEGORY_CODES)[number];
...
/** Keep in sync with apps/web school-admission-category.ts */
...
export function isSchoolCasteCategoryCode(
  value: string,
): value is SchoolCasteCategoryCode {
...
export function schoolCasteCategoryPolicy(
  ...
  if (!code) return null;
  return (
    SCHOOL_CASTE_CATEGORY_POLICY.find((item) => item.code === code) ?? null
  );
}
...
export function resolveSchoolCasteCategory(
  ...
  const rawCategory =
    typeof child?.category === 'string' ? child.category.trim() : '';
...
export function applySchoolCasteCategoryToChild(
  child: Record<string, unknown>,
): Record<string, unknown> {
  const categoryRaw =
    typeof child.category === 'string' ? child.category.trim() : '';
  if (categoryRaw && !isSchoolCasteCategoryCode(categoryRaw)) {
    throw new Error('INVALID_SCHOOL_CASTE_CATEGORY');
  }
  const resolved = resolveSchoolCasteCategory(child);
  if (!resolved) return child;
  return {
    ...child,
    category: resolved.code,
    caste: resolved.label,
  };
}
