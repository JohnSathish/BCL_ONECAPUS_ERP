 ...
 * Keep in sync with apps/web/lib/school-document-requirements.ts
 *
...
export type SchoolConditionalSlotCode =
  (typeof SCHOOL_CONDITIONAL_SLOT_CODES)[number];

export type SchoolDocumentRequirementRule = {
  /** Stable id for admin editing */
  id: string;
  slotCode: SchoolConditionalSlotCode;
  label: string;
  helperText: string;
  /** When set, child.community must match one of these (case-insensitive). */
  communities?: string[];
  /** When set, resolved category code must be one of these. */
  categories?: string[];
  required: boolean;
};
...
export function isLegacySchoolDocumentRequirements(
  ...
  const fatherCats = (father.categories ?? [])
    .map((c) => c.toUpperCase())
    .sort();
  const motherCommunities = (mother.communities ?? [])
    .map(normalizeToken)
    .sort();
  ...
  const fatherIncludesStAndObc =
    father.slotCode === 'FATHER_SC_OBC_CERT' &&
    fatherCats.join(',') === 'OBC,ST';
...
export function isMissingGeneralCasteDocumentRequirements(
  ...
  return (
    ids.join(',') ===
    'father_obc_for_obc,father_sc_for_sc,mother_st_for_st_tribes'
  );
}
...
export function normalizeSchoolDocumentRequirements(
  ...
  for (const item of rulesRaw) {
    ...
    if (
      !(SCHOOL_CONDITIONAL_SLOT_CODES as readonly string[]).includes(slotCode)
    ) {
      continue;
    }
...
function ruleMatches(
  ...
  if (hasCommunityGate) {
    ...
    communityOk = rule.communities!.some(
      (name) => normalizeToken(name) === community,
    );
  }
  ...
  if (hasCategoryGate) {
    ...
    categoryOk = rule.categories!.some(
      (code) => code.toUpperCase() === categoryCode.toUpperCase(),
    );
  }
...
/**
 * Returns only the certificate rules that apply to this applicant.
 * Does not include universally required slots (photo, birth cert, etc.).
 */
export function resolveApplicableSchoolCertificates(
  ...
  const categoryCode =
    ...
    (typeof child.category === 'string' ? child.category.trim() : null);
  const config = normalizeSchoolDocumentRequirements(
