...
export const STAFF_DOCUMENT_SLOTS: StaffDocumentSlot[] = [
  ...
  {
    code: 'SALARY_REVISION_ORDER',
    label: 'Salary Revision Orders',
    category: 'SERVICE',
  },
  { code: 'LEAVE_RECORDS', label: 'Leave Records', category: 'SERVICE' },
  {
    code: 'DISCIPLINARY_RECORDS',
  ...
  {
    code: 'QUARTER_SURRENDER_ORDER',
    label: 'Quarter Surrender Order',
    category: 'ACCOMMODATION',
  },
  {
    code: 'RENT_DEDUCTION_APPROVAL',
    label: 'Rent Deduction Approval',
    category: 'ACCOMMODATION',
  },
  {
...
export const STAFF_DOCUMENT_SLOT_CODES = new Set(
  STAFF_DOCUMENT_SLOTS.map((s) => s.code),
);
...
export function staffDocumentLabel(code: string) {
  return (
    STAFF_DOCUMENT_SLOTS.find((s) => s.code === code)?.label ??
    code.replace(/_/g, ' ')
  );
}
...
export function slotSupportsExpiry(code: string) {
  return (
    STAFF_DOCUMENT_SLOTS.find((s) => s.code === code)?.supportsExpiry ?? false
  );
}
