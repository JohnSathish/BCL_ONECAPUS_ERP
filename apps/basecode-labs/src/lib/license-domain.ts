...
/** Hosts / institution codes that identify the same St. Luke's installation. */
...
/**
 * True when no restriction is set, or any activation candidate matches
 * a comma-separated restriction (hostname or institution code).
 */
export function licenseDomainAllowed(
  ...
  const got = candidates.map((c) => (c ? normalizeHost(c) : '')).filter(Boolean);
  if (!got.length) return false;
  return allowedList.some((allowed) => got.some((g) => hostMatches(allowed, g)));
}
